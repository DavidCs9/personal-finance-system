...
export const handler: DynamoDBStreamHandler = async (event) => {
  for (const record of event.Records) {
    ...
    const dispatchKey = record.dynamodb.NewImage.SK?.S;
    const job = record.dynamodb.NewImage.job?.M;
    if (!exceptionId || !dispatchKey || !job) continue;
    ...
    await database.send(new UpdateCommand({ TableName: tableName, Key: { PK: `RETRY#${exceptionId}`, SK: dispatchKey }, UpdateExpression: 'SET #status = :status, dispatchedAt = :at', ExpressionAttributeNames: { '#status': 'status' }, ExpressionAttributeValues: { ':status': 'dispatched', ':at': new Date().toISOString() } }));
  }
};
