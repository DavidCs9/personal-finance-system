...
import { detectEmailInstitution, header, shouldIgnoreEmail } from '@finance/ingestion';
import { EmailTextractError, extractPurchaseFromEmailMime } from './email-textract.js';
...
const ingest = async (job: IngestionJob): Promise<void> => {
  ...
  if (!institution) {
    await saveException({
      ...
      details: 'No configured institution accepted this SES-received email.',
      source,
    }, dedupeKey);
    return;
  }
  ...
  let parsed;
  try {
    parsed = await extractPurchaseFromEmailMime({
      textract,
      s3,
      mime,
    ...
    const reason = error instanceof EmailTextractError
