...
function SignIn({ onSignedIn }: { onSignedIn(token: string): void }) {
  ...
  return <main className="auth-shell"><section className="auth-card"><div className="brand"><span className="brand-mark">L</span><span>Ledger</span><small>personal</small></div><h1>{challenge ? "Elige tu contraseña" : "Tu registro financiero"}</h1><p>{challenge ? "Es tu primer acceso. Cognito requiere reemplazar la contraseña temporal que llegó por correo." : "Accede con tu correo y la contraseña temporal enviada por AWS."}</p><form onSubmit={challenge ? complete : submit}>{!challenge && <><label>Correo<input type="email" value={email} onChange={(event) => setEmail(event.target.value)} required autoComplete="email" /></label><label>Contraseña temporal<input type="password" value={password} onChange={(event) => setPassword(event.target.value)} required autoComplete="current-password" /></label></>}{challenge && <label>Nueva contraseña<input type="password" value={newPassword} onChange={(event) => setNewPassword(event.target.value)} minLength={14} required autoComplete="new-password" /></label>}{error && <p className="form-error">{error}</p>}<button type="submit" disabled={busy}>{busy ? "Un momento..." : challenge ? "Guardar y entrar" : "Entrar"}</button></form></section></main>;
}
