...
export const ledgerApi = {
  ...
  async analytics(month: string, idToken: string): Promise<SpendingAnalytics> {
    return request<SpendingAnalytics>(`/analytics?month=${encodeURIComponent(month)}`, idToken);
  },
  ...
  async setEventTags(eventId: string, tags: readonly string[], idToken: string): Promise<PurchaseEvent> {
    return request<PurchaseEvent>(`/events/${encodeURIComponent(eventId)}`, idToken, {
      method: "PATCH",
      headers: { "Content-Type": "application/json" },
