...
- Agent tag and category edits default to accepted movements, exclude rejected movements, freeze the exact affected IDs before apply, create one revision per movement, and remain idempotent and undoable. Range dates are inclusive in the finance timezone. Category edits require exact event IDs, or a range plus an explicit merchant/current-category/uncategorized filter, and preview every affected movement. Agent category edits never update merchant rules.
