...
- The **assistant** is a global sheet opened from the topbar (not a fourth tab). Its visible thread clears when the sheet closes or the month changes, while durable conversational memory remains user-scoped, visible, and deletable inside the sheet. It may prepare bulk tag/category edits, but it must show an exact preview and require one authenticated confirmation before the ledger API applies an auditable operation; it never receives a generic database-write tool. Ground answers in tool results; show citation chips. Private mode blurs amounts in the sheet too. See [`../../docs/ai-assistant.md`](../../docs/ai-assistant.md).
...
- Bulk edits default to accepted movements, exclude rejected movements, freeze the exact affected IDs before confirmation, create one revision per movement, and remain idempotent and undoable. Range dates are inclusive in the finance timezone. Bulk category edits never update merchant rules.
