...
- The **assistant** is a global sheet opened from the topbar (not a fourth tab). Its visible thread clears when the sheet closes or the month changes, while durable conversational memory remains user-scoped, visible, and deletable inside the sheet. An explicit chat instruction to add or remove tags authorizes that bounded change: freeze the exact IDs internally, apply it in the same agent turn without a second UI confirmation, then show a factual receipt and allow undo by chat. Category changes still require UI confirmation. The assistant never receives a generic database-write tool. Ground answers in tool results; show citation chips. Private mode blurs amounts in the sheet too. See [`../../docs/ai-assistant.md`](../../docs/ai-assistant.md).
...
- Agent tag edits default to accepted movements, exclude rejected movements, freeze the exact affected IDs before apply, create one revision per movement, and remain idempotent and undoable. Range dates are inclusive in the finance timezone. Bulk category edits remain outside this direct agent path and never update merchant rules.
